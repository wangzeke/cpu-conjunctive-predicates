     ...
     // printf("bit_width = %d\n", BIT_WIDTH_BYTESLICE);
		...
		uint64_t t3 = thread_time();
		 ...
		 {		
		   ...
		   block2_2->Scan(Comparator::kLess, literal_3, bvblock, Bitwise::kAnd);
         }
		 ...
		 if (execution_model == 2)
		 {		
 		   block2_1->Scan(Comparator::kEqual,literal_2, bvblock, Bitwise::kSet);
           block2->Scan(  Comparator::kEqual,  literal_1, bvblock, Bitwise::kAnd);
		   block2_2->Scan(Comparator::kLess, literal_3, bvblock, Bitwise::kAnd);
         }
		 else if (execution_model == 3)
		 {		
		   block2_2->Scan(Comparator::kLess, literal_3, bvblock, Bitwise::kSet);
           block2->Scan(  Comparator::kEqual,literal_1, bvblock, Bitwise::kAnd);
 		   block2_1->Scan(Comparator::kEqual,literal_2, bvblock, Bitwise::kAnd);
         }
...
void main(int argc, char **argv)
{
	...
	size_t tuples        = 60490115;//1000 * 1000 * 1000; //59987860; //
	int b, barriers      = 3* 7;
