...
void *run(void *arg)
{
	...
	{
        uint32_t literal_1   = 2109;  //1995-9-1
	    uint32_t literal_2   = 2140;  //1995-10-1
     ...
     // printf("bit_width = %d\n", BIT_WIDTH_BYTESLICE);
		...
		uint64_t t3 = thread_time();
         ...
         else if (execution_model == 1)
		 {
		   ...
		   block2->Scan(Comparator::kGreater,literal_1, bvblock, Bitwise::kAnd);
		 }
/*		else if (execution_model == 2)
		 {
		   block2_1->Scan(Comparator::kLess, literal_2, bvblock, Bitwise::kSet);
		 ...
		 {
		   block2_2->Scan(Comparator::kGreater, literal_3, bvblock, Bitwise::kSet);
           block2->Scan(Comparator::kEqual, literal_1, bvblock, Bitwise::kAnd);
	...
	{   
		 for (size_t ii = 0; ii < num_; ii++) //
		{
            ...
            if (real !=  eval )
			{
		      ...
		      printf("%d:  eval: %d, real: %d (%d, %d)\n", ii, eval, real, (original[ii] > literal_1), (original[ii] < literal_2));
#else 
