...
void *run(void *arg)
{
	...
	//for (bits = min_bits ; bits <= max_bits ; ++bits) 
	{
        ...
        uint32_t literal_3   = (uint32_t) ( (1-T3_selevitity) * (float)((1<<bits)-1) ); //do greater than literal...
	    uint32_t literal_4   = (uint32_t) (    T4_selevitity  * (float)((1<<bits)-1) );   //do less    than literal...
    ...
    //ByteSliceColumnBlock<16>* block2 = new ByteSliceColumnBlock<16>(num_);
      ...
      ByteSliceColumnBlock<BIT_WIDTH_BYTESLICE>* block2_2 = new ByteSliceColumnBlock<BIT_WIDTH_BYTESLICE>(num_);
      ByteSliceColumnBlock<BIT_WIDTH_BYTESLICE>* block2_3 = new ByteSliceColumnBlock<BIT_WIDTH_BYTESLICE>(num_);
...
//	  pthread_barrier_wait(barrier++);
	...
	{   
		...
		uint64_t t3 = thread_time();
		   ...
		   block2_2->Scan(Comparator::kGreater, literal_3, bvblock_1, Bitwise::kSet);
		   block2_3->Scan(Comparator::kLess,    literal_4, bvblock_1, Bitwise::kAnd);
	...
	{   
		...
		{
			//size_t ii = 11;
			bool real  = ((original[ii] > literal_1)&&(original_1[ii] < literal_2))||((original_2[ii] > literal_3)&&(original_3[ii] < literal_4)); 
	
			bool eval  = bvblock->GetBit(ii);
            if (real !=  eval )
			{
		      printf("%d:  eval: %d, real: %d (%d, %d, %d, %d)\n", ii, eval, real, 
			  (original[ii] > literal_1), (original_1[ii] < literal_2), (original_2[ii] > literal_3), (original_3[ii] < literal_4));
              break;
			 //   return NULL;
			}
  	    }
...
void main(int argc, char **argv)
{
    ...
    float T4_selevitity  = argc > 5 ? atof(argv[5])  : 0.5; 
	...
	printf("Threads: %d, T1_selevitity = %f, T2_selevitity = %f, T3_selevitity = %f, , T4_selevitity = %f\n", threads, T1_selevitity, T2_selevitity, T3_selevitity, T4_selevitity);
