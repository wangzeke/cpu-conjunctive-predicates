...
void Q10_cmp_with_literal_P_S(WordUnit* bitmap, WordUnit len, 
								  ...
								  ) 
{
	...
	size_t kNumBytesPerCode_3 = 2;
	size_t kNumPaddingBits_3  = kNumPaddingBits_2;
...
*/
			if ( (kNumBytesPerCode_2 > 1) && ( (!avx_iszero( m_equal_2))||(!avx_iszero( m_equal_3)) ) ) //for the remaining segments of predicate 2.
			{
...
void *run(void *arg)
{
    ...
    uint32_t literal_1   = 3;
    uint32_t literal_2   = 1396; //1993-10-01
    uint32_t literal_3   = 1488; //1994-01-01
...
//	  pthread_barrier_wait(barrier++);
  ...
  //1....................:l_returnflag
      if((fp_r_name=fopen("../../lineitemWT/output_l_returnflag.txt","r"))==NULL) {
        printf("cannot open output_l_returnflag.txt/n");
        exit(1);
      }
	  i = 0;
   ...
   pthread_barrier_wait(barrier++);
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
	  pthread_barrier_wait(barrier++);
		uint64_t t1 = thread_time(); //
                    ...
                    Q10_cmp_with_literal_P_S(bitvector, T1_len, 
    ...
    ///////////////fourth barrier to make sure no too much noise comes from the other threads when the thread collects the statistics/////////////////////		
	 ...
	 {   
		...
		{
            bool real  = ( (original_1[ii] == literal_1)&&(original_2[ii] >= literal_2) && (original_2[ii] < literal_3)); 
