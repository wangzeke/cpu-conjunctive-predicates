...
void *run(void *arg)
{
	...
	uint32_t literal_2           = d->literal_2;//(uint32_t) (    T2_selevitity  * (float)((1<<T2_bit_width)-1) );   //do less    than literal...
  ...
  ///////////////////////allocate space for input data and output bit vector///////////////////////////////////////	
   ...
   for (i = 0; i < 4; i++) // malloc memory space for two codes. 
   {
	...
	uint32_t*zipf_data       = (uint32_t*)malloc(T1_len_aligned*sizeof(uint32_t));
...
{	
	...
	if (d->thread == 0)
	{
	  printf("p_s_model = %d, T1_selevitity = %f, T2_selevitity = %f\n", p_s_model, T1_selevitity, T2_selevitity);
	}
   ...
   pthread_barrier_wait(barrier++);
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
      ...
      //do the job.....	
	  if (p_s_model == 0)
        two_columns_cmp_with_literal_nP_nS(bitvector, T1_len, 
      ...
      //column_cmp_with_literal_nP_nS(data_, literal, bitvector, T1_len, kNumBytesPerCode, kNumPaddingBits); 
        ...
        two_columns_cmp_with_literal_nP_S(bitvector, T1_len, 
                                           data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								           data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								          );  
	  else if (p_s_model == 2)
        two_columns_cmp_with_literal_P_nS(bitvector, T1_len, 
                                           data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								           data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								          ); 
	  else if (p_s_model == 3)
        two_columns_cmp_with_literal_P_S(bitvector, T1_len, 
                                           data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								           data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								          ); 									 
/*	  else if (p_s_model == 4)
       two_columns_cmp_with_literal_pf_nP_nS(data_, bitvector, T1_len, 
                                       literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								       literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								         );  
      //column_cmp_with_literal_nP_nS(data_, literal, bitvector, T1_len, kNumBytesPerCode, kNumPaddingBits); 
	  else if (p_s_model == 5)
       two_columns_cmp_with_literal_pf_nP_S(data_, bitvector, T1_len, 
                                       literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								       literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								        );  
	  else if (p_s_model == 6)
       two_columns_cmp_with_literal_pf_P_nS(data_, bitvector, T1_len, 
                                       literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								       literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								        );  
	  else if (p_s_model == 7)
       two_columns_cmp_with_literal_pf_P_S(data_, bitvector, T1_len, 
                                       literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								       literal_2, kNumBytesPerCode_2, kNumPaddingBits_2
								       ); 
*/									   
...
void main(int argc, char **argv)
{
  ...
  uint32_t  prefetch_model = argc > 5 ? atoi(argv[5]) : 0;                  //default: enable prefetcher
  ...
  float     T1_selevitity  = argc > 8 ? atof(argv[8]) : 0.5; 
  float     T2_selevitity  = argc > 9 ? atof(argv[9]) : 0.5; 
  uint32_t  literal_2      = argc > 10? atoi(argv[10]): 1000;               //default bit width for 2nd column
