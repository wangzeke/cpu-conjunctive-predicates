 ...
 * 
 * Description: To implement Q17 under Hebe: a deterministic execution scheme.
 *******************************************************************************/
#include <cstdio>
#include <cstdlib>
#include <time.h>
...
void Q17_cmp_with_literal_P_S(WordUnit* bitmap, WordUnit len, 
								  ...
								  ) 
{
    ...
    AvxUnit mask_literal_3[kNumBytesPerCode_1];	
/*    
	literal_1 <<= kNumPaddingBits_1;
    literal_2 <<= kNumPaddingBits_1;
    literal_3 <<= kNumPaddingBits_1;	
*/
    for(size_t byte_id=0; byte_id < kNumBytesPerCode_1; byte_id++){
         ...
         mask_literal_2[byte_id] = avx_set1<ByteUnit>(byte);		 
		          byte           = FLIP(static_cast<ByteUnit>(literal_3 >> 8*(kNumBytesPerCode_1 - 1 - byte_id)));
...
void *run(void *arg)
{
	...
	int kNumPaddingBits_3        = kNumBytesPerCode_3 * 8 - T3_bit_width;
        ...
        uint32_t literal_1   = 8;    //p_brand = Brand#23  
	    uint32_t literal_2   = 18;   //p_container = MED BOX
        uint32_t literal_3   = 6;    //qualility: 6
  ...
  //  printf("after set data_.\n");  
   ...
   pthread_barrier_wait(barrier++);
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
     ...
     Q17_cmp_with_literal_P_S(bitvector, T1_len, 
...
void main(int argc, char **argv)
{
  ...
  uint64_t tuples          = 59987861;//1000000000; //60490115;  //
