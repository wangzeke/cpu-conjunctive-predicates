 ...
 * 
 * Description: To implement Q7 under fine-grained processing model..........
 *******************************************************************************/
#include <cstdio>
#include <cstdlib>
#include <time.h>
...
void Q7_cmp_with_literal_P_S(WordUnit* bitmap, WordUnit len, 
								  ...
								  ) 
{
    ...
    for(size_t offset = 0, bv_word_id = 0; offset < len; offset += NUM_WORD_BITS, bv_word_id++)
	{
		...
		{  
            AvxUnit m_less_3,    m_less_4;
            AvxUnit m_greater_3, m_greater_4;
            AvxUnit m_equal_3,   m_equal_4;
			AvxUnit m_success_3, m_success_4;
			AvxUnit m_fail_3,    m_fail_4;
            AvxUnit m_less_a,    m_less_b,    m_less_c,    m_less_d ;
            AvxUnit m_greater_a, m_greater_b, m_greater_c, m_greater_d;
            AvxUnit m_equal_a,   m_equal_b,   m_equal_c,   m_equal_d; 			
			AvxUnit m_success_a, m_success_b, m_success_c, m_success_d;
			AvxUnit m_fail_a,    m_fail_b,    m_fail_c,    m_fail_d;
           ...
           // bool move_to_next_segment = true;
            ...
            computeKernelWithMask_FIRST<FIRST_COMPARISON_TYPE>(avx_load( (void *)(data_1[0]+offset+i)),
                                                          ...
                                                          m_success_a,
                                                          m_fail_a); 
            computeKernelWithMask_FIRST<FIRST_COMPARISON_TYPE>(avx_load( (void *)(data_2[0]+offset+i)),
                                                          mask_literal_2[0],
                                                          m_less_b,
                                                          m_greater_b,
                                                          m_equal_b,
            ...
            tmp_result = avx_or( avx_and(m_equal_a, m_equal_b), avx_and(m_equal_c, m_equal_d));
														  
            computeKernelWithMask_FIRST<SECOND_COMPARISON_TYPE>(avx_load( (void *)(data_3[0]+offset+i)),
                                                          mask_literal_3[0],
                                                          m_less_3,
                                                          m_greater_3,
                                                          m_equal_3,
          ...
          //aggregating the information from all the columns.//// 
              ...
              computeForEarlyStop<PREDICATE_TYPE>(m_equal_4,
                                                  m_success_4,
                                                  m_fail_4,
                                                  m_equal_3,
                                                  m_success_3,
		  ...
		  {      
			...
			if ( (kNumBytesPerCode_3 > 1) && ( !( avx_iszero(m_equal_3) && avx_iszero(m_equal_4) ) ) ) //for the remaining segments of predicate 3.
			{
            ...
            //AvxUnit m_result         = avx_and(m_less_2, m_greater_1);
			   AvxUnit m_result_1, m_result_2, m_result_3, m_result_4, m_result_tmp, m_result;
               ...
               computeConjunctivePredicates<PREDICATE_TYPE>(m_result_3,
                                                            m_result_4,
							                                m_result_tmp);
               computeConjunctivePredicates<PREDICATE_TYPE>(m_result_tmp,
                                                            tmp_result,
							                                m_result);
...
void *run(void *arg)
{
	...
	int kNumPaddingBits_3        = kNumBytesPerCode_3 * 8 - T3_bit_width;
        ...
        uint32_t literal_1   = 9;  //Germany
        uint32_t literal_2   = 8;  //France
	    uint32_t literal_3   = 1861; //1995-01-01
        uint32_t literal_4   = 2604; //1996-12-31
  ...
  ///////////////////////allocate space for input data and output bit vector///////////////////////////////////////	
   ...
   //  printf("before set tuple.\n"); 
	  ...
	  FILE *fp_n1_name, *fp_n2_name, *fp_l_shipdate;//
	  char str[128];
	  uint32_t tmp;
  //1....................: output_n1_name
      if((fp_n1_name=fopen("../../lineitemWT/output_n1_name.txt","r"))==NULL) {
        printf("cannot open output_n1_name.txt/n");
        exit(1);
      }
	  i = 0;
      while(fscanf(fp_n1_name, "%d\n", &tmp) > 0) { 
         original_1[i]       = tmp; //tmp_cons; //
         SetTuple(data_1,  i, tmp,  kNumBytesPerCode_1, kNumPaddingBits_1);//block2_2->SetTuple(i, tmp);		  
	     i++;
      }
      fclose(fp_n1_name);
  //2....................: output_n2_name.txt
      if((fp_n2_name=fopen("../../lineitemWT/output_n2_name.txt","r"))==NULL) {
        printf("cannot open output_n2_name.txt/n");
        exit(1);
      }
	  i = 0;
      while(fscanf(fp_n2_name, "%d\n", &tmp) > 0) { 
         original_2[i]       = tmp; //tmp_cons; //
         SetTuple(data_2,  i, tmp,  kNumBytesPerCode_2, kNumPaddingBits_2);//block2_2->SetTuple(i, tmp);		  
	  
	     i++;
      }
      fclose(fp_n2_name);
  //3....................: output_l_shipdate.txt
      ...
      while(fscanf(fp_l_shipdate, "%d\n", &tmp) > 0) { 
         ...
         SetTuple(data_3,  i, tmp,  kNumBytesPerCode_3, kNumPaddingBits_3);//block2_2->SetTuple(i, tmp);		  
	     i++;
      }
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
	  pthread_barrier_wait(barrier++);
		uint64_t t1 = thread_time(); //
                    ...
                    Q7_cmp_with_literal_P_S(bitvector, T1_len, 
    ...
    ///////////////fourth barrier to make sure no too much noise comes from the other threads when the thread collects the statistics/////////////////////		
	 ...
	 {   
		 for (size_t ii = 0; ii < T1_len; ii++) //
		{
			...
			bool real  = ( 
			               ( ((original_2[ii] == literal_1)&&(original_1[ii] == literal_2)) ||
			                 ((original_2[ii] == literal_2)&&(original_1[ii] == literal_1)) 
			               ) &&
