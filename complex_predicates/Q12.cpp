 ...
 * 
 * Description: To implement Q12 under fine-grained batch processing of predicates.
 *******************************************************************************/
#include <cstdio>
#include <cstdlib>
#include <time.h>
...
#define FIRST_COMPARISON_TYPE  Comparator::kEqual //for first two cases....

#define SECOND_COMPARISON_TYPE Comparator::kLess
#define THIRD_COMPARISON_TYPE  Comparator::kLess //
#define FOURTH_COMPARISON_TYPE Comparator::kGreaterEqual    //
#define FIFTH_COMPARISON_TYPE  Comparator::kLess    //


#define PREDICATE_TYPE Bitwise::kAnd
	 
	 
void Q12_cmp_with_literal_P_S(WordUnit* bitmap, WordUnit len, 
								  ...
								  ) 
{
    ...
    for(size_t byte_id=0; byte_id < kNumBytesPerCode_1; byte_id++){
         ...
         mask_literal_1[byte_id] = avx_set1<ByteUnit>(byte);
                  byte           = FLIP(static_cast<ByteUnit>(literal_2 >> 8*(kNumBytesPerCode_1 - 1 - byte_id)));
    ...
    for(size_t byte_id=0; byte_id < kNumBytesPerCode_3; byte_id++){
         ...
         mask_literal_3[byte_id] = avx_set1<ByteUnit>(byte);			 
		          byte           = FLIP(static_cast<ByteUnit>(literal_4 >> 8*(kNumBytesPerCode_3 - 1 - byte_id)));
    ...
    for(size_t offset = 0, bv_word_id = 0; offset < len; offset += NUM_WORD_BITS, bv_word_id++)
	{
		...
		{  
            ...
            AvxUnit m_less_a,    m_less_b ;
            AvxUnit m_greater_a, m_greater_b ;
            AvxUnit m_equal_a,   m_equal_b ; 			
			AvxUnit m_success_a, m_success_b ;
			AvxUnit m_fail_a,    m_fail_b ;
			
			AvxUnit tmpA, tmpB, tmp_result;
           ...
           // bool move_to_next_segment = true;
			...
			AvxUnit v_data_1 =  avx_load( (void *)(data_1[0]+offset+i));
			AvxUnit v_data_2 =  avx_load( (void *)(data_2[0]+offset+i));
			AvxUnit v_data_3 =  avx_load( (void *)(data_3[0]+offset+i));
			AvxUnit v_data_4 =  avx_load( (void *)(data_4[0]+offset+i));
			
	        ////////////////////////Compute the shipmodel with type: IN./////////////////////////////////
            computeKernelWithMask_FIRST<FIRST_COMPARISON_TYPE>(v_data_1,    //avx_load( (void *)(data_1[0]+offset+i)),
                                                          mask_literal_1[0],
                                                          m_less_a,
                                                          m_greater_a,
                                                          m_equal_a,
			...
			//used for final result and for pruning											  
            tmp_result = avx_or(m_equal_a, m_equal_b);

			
            computeKernelWithMask_FIRST<SECOND_COMPARISON_TYPE>(v_data_2,  //avx_load( (void *)(data_2[0]+offset+i)),
                                                                v_data_3,  //avx_load( (void *)(data_3[0]+offset+i)),  //mask_literal_2[0],
                                                                m_less_2,
                                                                m_greater_2,
                                                                m_equal_2,
                                                                m_success_2,
            ...
            computeKernelWithMask_FIRST<THIRD_COMPARISON_TYPE>(v_data_4,   //avx_load( (void *)(data_4[0]+offset+i)),
                                                               v_data_2,   //avx_load( (void *)(data_2[0]+offset+i)),
                                                               m_less_3,
                                                               m_greater_3,
                                                               m_equal_3,
                                                               m_success_3,
            ...
            computeKernelWithMask_FIRST<FOURTH_COMPARISON_TYPE>(v_data_3,   //avx_load( (void *)(data_3[0]+offset+i)),
                                                          mask_literal_3[0],
                                                          m_less_4,
                                                          m_greater_4,
                                                          m_equal_4,
            ...
            computeKernelWithMask_FIRST<FIFTH_COMPARISON_TYPE>(v_data_3,   //avx_load( (void *)(data_3[0]+offset+i)),
                                                          mask_literal_4[0],
                                                          m_less_5,
                                                          m_greater_5,
                                                          m_equal_5,
          ...
          //aggregating the information from all the columns.//// 
              ...
              computeForEarlyStop<PREDICATE_TYPE>(m_equal_5,
                                                  m_success_5,
                                                  m_fail_5,
                                                  m_equal_2,
                                                  m_success_2,
			...
			// further pruning with the first predicate...									       
			agg_equal = avx_and(agg_equal, tmp_result);									  
		  ...
		  {      
              ...
              m_equal_5 = avx_and(m_equal_5, agg_equal);
			  
			if ( (kNumBytesPerCode_2 > 1) && !avx_iszero( m_equal_2) ) //for the remaining segments of predicate 2.
			{
                computeKernelWithMask<SECOND_COMPARISON_TYPE>( avx_load( (void *)(data_2[1]+offset+i)),
                                                               avx_load( (void *)(data_3[1]+offset+i)),
                                                               m_less_2,
                                                               m_greater_2,
                                                               m_equal_2,
                                                               m_success_2,
			...
			if ( (kNumBytesPerCode_2 > 1) && !avx_iszero( m_equal_3) ) //for the remaining segments of predicate 2.
			{
                computeKernelWithMask<THIRD_COMPARISON_TYPE>( avx_load( (void *)(data_4[1]+offset+i)),
                                                               avx_load( (void *)(data_2[1]+offset+i)),
                                                               m_less_3,
                                                               m_greater_3,
                                                               m_equal_3,
                                                               m_success_3,
			...
			if ( (kNumBytesPerCode_2 > 1) && ((!avx_iszero( m_equal_4)) || (!avx_iszero( m_equal_5))) ) //for the remaining segments of predicate 2.
			{
                computeKernelWithMask<FOURTH_COMPARISON_TYPE>( avx_load( (void *)(data_3[1]+offset+i)),
                                                               mask_literal_3[1],
                                                               m_less_4,
                                                               m_greater_4,
                                                               m_equal_4,
                                                               m_success_4,
                                                               m_fail_4);
                computeKernelWithMask<FIFTH_COMPARISON_TYPE>( avx_load( (void *)(data_3[1]+offset+i)),
                                                               mask_literal_4[1],
                                                               m_less_5,
                                                               m_greater_5,
                                                               m_equal_5,
            ...
            /////////////combine the result::::generat the vector result for the above computing....//////////////////////////////
               ...
               computeMask_one_predicate<FOURTH_COMPARISON_TYPE>(
                                 ...
                                 m_less_4,
			                     m_result_4 );		
               computeMask_one_predicate<FIFTH_COMPARISON_TYPE>(
							     m_equal_5,
                                 m_greater_5,
                                 m_less_5,
			                     m_result_5 );									 
               computeConjunctivePredicates<PREDICATE_TYPE>(m_result_5,
                                                            m_result_2,
							                                m_result_tmp_2);
			   ...
			   m_result = avx_and(m_result, tmp_result);
...
void *run(void *arg)
{
	...
	uint32_t T3_bit_width      = 12;//d->T3_bit_width;
	uint32_t T4_bit_width      = 12;//d->T4_bit_width;
	...
	int kNumPaddingBits_4        = kNumBytesPerCode_4 * 8 - T4_bit_width;
        ...
        uint32_t literal_1   = 3;    //MAIL
	    uint32_t literal_2   = 6;    //SHIP
        uint32_t literal_3   = 1488; //1994-1-1  
        uint32_t literal_4   = 1861; //1995-1-1  
		
		
  ///////////////////////Generate the input output data..///////////////////////////////////////	
    uint32_t *original_1, *original_2, *original_3, *original_4;  // input original data.  
	ByteUnit *data_1[4], *data_2[4], *data_3[4], *data_4[4];     //
...
//	  pthread_barrier_wait(barrier++); 
	  FILE *fp_l_shipmode, *fp_l_commitdate, *fp_l_receiptdate, *fp_l_shipdate;//
	  char str[128];
	  uint32_t tmp;
      
  //1....................: output_l_shipmode.txt
  ...
  //2....................: output_l_commitdate.txt
      if((fp_l_commitdate=fopen("../../lineitemWT/output_l_commitdate.txt","r"))==NULL) {
        printf("cannot open output_l_commitdate.txt/n");
        exit(1);
      }
	  i = 0;
      while(fscanf(fp_l_commitdate, "%d\n", &tmp) > 0) { 
         original_2[i]       = tmp; //tmp_cons; //
         SetTuple(data_2,  i, tmp,  kNumBytesPerCode_2, kNumPaddingBits_2);			  
	     i++;
    }
    fclose(fp_l_commitdate);
  //3....................: output_l_receiptdate.txt
      if((fp_l_receiptdate=fopen("../../lineitemWT/output_l_receiptdate.txt","r"))==NULL) {
        printf("cannot open output_l_receiptdate.txt/n");
        exit(1);
      }
	  i = 0;
      while(fscanf(fp_l_receiptdate, "%d\n", &tmp) > 0) { 
         original_3[i]       = tmp; //tmp_cons; //
         SetTuple(data_3,  i, tmp,  kNumBytesPerCode_3, kNumPaddingBits_3);			  
	     i++;
    }
    fclose(fp_l_receiptdate);
	
 
  //4....................: output_l_shipdate.txt
      ...
      while(fscanf(fp_l_shipdate, "%d\n", &tmp) > 0) { 
         ...
         SetTuple(data_4,  i, tmp,  kNumBytesPerCode_4, kNumPaddingBits_4);			  
	     i++;
    }
   ...
   pthread_barrier_wait(barrier++);
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
	  pthread_barrier_wait(barrier++);
       ...
       Q12_cmp_with_literal_P_S(bitvector, T1_len, 
	 ...
	 {   
		 for (size_t ii = 0; ii < T1_len; ii++) //
		{
			bool real =( ((original_1[ii] == literal_1)||(original_1[ii] == literal_2))&&
			              (original_2[ii] < original_3[ii])&&
			              (original_4[ii] < original_2[ii])&&
						 ((original_3[ii] >= literal_3)&&(original_3[ii] < literal_4))
						  ); 				   
