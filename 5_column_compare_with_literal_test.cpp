...
#include "5_column_compare_with_literal.h" 
...
typedef struct {
    ...
    uint32_t T5_bit_width; //bit width for different columns in the table T5.
...
void *run(void *arg)
{
	...
	uint32_t T5_bit_width      = d->T5_bit_width;
	...
	float    T5_selevitity     = d->T5_selevitity;
    ...
    int kNumBytesPerCode_5       = (T5_bit_width+7)/8;
	int kNumPaddingBits_5        = kNumBytesPerCode_5 * 8 - T5_bit_width;
...
/*	
	...
	uint32_t literal_4           = rand32_next(gen) & ( (1<<T4_bit_width) - 1 ); //do less    than literal...
*/
	...
	uint32_t literal_4           = (uint32_t) (    T4_selevitity  * (float)((1<<T4_bit_width)-1) );   //do less    than literal...
	uint32_t literal_5           = (uint32_t) (    T5_selevitity  * (float)((1<<T5_bit_width)-1) );   //do less    than literal...

  ///////////////////////Generate the input output data..///////////////////////////////////////	
    uint32_t *original_1, *original_2, *original_3, *original_4, *original_5;  // input original data.  
	ByteUnit *data_1[4], *data_2[4], *data_3[4], *data_4[4], *data_5[4]; //*data_2[4];  //compressed to byte boundary.
  ...
  ///////////////////////allocate space for input data and output bit vector///////////////////////////////////////	
   ...
   if (original_5 == NULL) 
   {
         printf ( "input original_malloc 5 fails\n");
         return NULL;
   }      
   ...
   for(i=0; i < T1_len; i++){
      ...
      original_3[i]  = tmp_3; //
      uint32_t tmp_4 = rand32_next(gen) & ( (1<<T3_bit_width) - 1 ); 
      original_4[i]  = tmp_4; //
      uint32_t tmp_5 = rand32_next(gen) & ( (1<<T3_bit_width) - 1 ); 
      original_5[i]  = tmp_5; //
      ...
      SetTuple(data_4,  i, tmp_4,  kNumBytesPerCode_4, kNumPaddingBits_4);
      SetTuple(data_5,  i, tmp_5,  kNumBytesPerCode_5, kNumPaddingBits_5);
...
*/	
  ...
  {	
    ...
    ///////////////////////second barrier.to sync all the threads then begin to execute the code./////////////////////		
	  ...
	  if (p_s_model == 0)
        five_columns_cmp_with_literal_nP_nS(bitvector, T1_len, 
                                            ...
                                            data_5, literal_5, kNumBytesPerCode_5, kNumPaddingBits_5
								         );  
/*	  else if (p_s_model == 1)
        ...
        four_columns_cmp_with_literal_P_S(bitvector, T1_len, 
                                            data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								            data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2,
                                            data_3, literal_3, kNumBytesPerCode_3, kNumPaddingBits_3,
                                            data_4, literal_4, kNumBytesPerCode_4, kNumPaddingBits_4
        ...
        four_columns_c_cmp_with_literal_nP_nS(bitvector, T1_len, 
                                            data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								            data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2,
                                            data_3, literal_3, kNumBytesPerCode_3, kNumPaddingBits_3,
                                            data_4, literal_4, kNumBytesPerCode_4, kNumPaddingBits_4
        ...
        four_columns_c_cmp_with_literal_nP_S(bitvector, T1_len, 
                                            data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								            data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2,
                                            data_3, literal_3, kNumBytesPerCode_3, kNumPaddingBits_3,
                                            data_4, literal_4, kNumBytesPerCode_4, kNumPaddingBits_4
        ...
        four_columns_c_cmp_with_literal_P_nS(bitvector, T1_len, 
                                            data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								            data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2,
                                            data_3, literal_3, kNumBytesPerCode_3, kNumPaddingBits_3,
                                            data_4, literal_4, kNumBytesPerCode_4, kNumPaddingBits_4
        ...
        four_columns_c_cmp_with_literal_P_S(bitvector, T1_len, 
                                            data_1, literal_1, kNumBytesPerCode_1, kNumPaddingBits_1,
								            data_2, literal_2, kNumBytesPerCode_2, kNumPaddingBits_2,
                                            data_3, literal_3, kNumBytesPerCode_3, kNumPaddingBits_3,
                                            data_4, literal_4, kNumBytesPerCode_4, kNumPaddingBits_4
...
*/									 
	 ...
	 {   
		 for (size_t ii = 0; ii < T1_len; ii++) //
		{
			...
			if ( (p_s_model >= 4) && (p_s_model <= 7) )
               ...
               real  = ( (original_1[ii] > literal_1) && (original_2[ii] < literal_2) && (original_3[ii] > literal_3) && (original_4[ii] < literal_4) && (original_5[ii] > literal_5)); 
	...
	if (d->thread == 0) {
		...
		printf("%2d-%2d-%2d-%2d-%2d-bit codes, time: %6.3f, codes_per_ns: %6.3f\n", T1_bit_width, T2_bit_width, T3_bit_width, T4_bit_width, T5_bit_width, ((double)t1 / (double)d->threads), 
...
void main(int argc, char **argv)
{
  ...
  uint32_t  T5_bit_width   = argc > 7 ? atoi(argv[7]) : 17;                 //default bit width for 5th column
   
  uint32_t  prefetch_model = argc > 8 ? atoi(argv[8 ]) : 0;                  //default: enable prefetcher
  uint32_t  p_s_model_start= argc > 9 ? atoi(argv[9 ]) : 0;                  //0:nP_nS, 1:nP_S, 2:P_ns, 3:P_S,   
  uint32_t  p_s_model_end  = argc > 10? atoi(argv[10]) : 0;                  //4:pf_nP_nS, 1:pf_nP_S, 2:pf_P_ns, 3:pf_P_S,   
  float     T1_selevitity  = argc > 11? atof(argv[11]) : 0.5; 
  float     T2_selevitity  = argc > 12? atof(argv[12]) : 0.5; 
  float     T3_selevitity  = argc > 13? atof(argv[13]) : 0.5; 
  float     T4_selevitity  = argc > 14? atof(argv[14]) : 0.5; 
  float     T5_selevitity  = argc > 15? atof(argv[15]) : 0.5; 
  ...
  printf("tuples = %d, thread number = %d, huge_table_enable = %d, T1_bit_width = %d, T2_bit_width = %d, T3_bit_width = %d, T4_bit_width = %d, T5_bit_width = %d, prefetch_model = %d\n", 
         tuples,       thread_num,         huge_table_enable,      T1_bit_width,      T2_bit_width,      T3_bit_width,      T4_bit_width,      T5_bit_width,     prefetch_model     ); 
  ...
  uint64_t size_for_each_thread = compute_task_len_for_each_thread(task_len, tuples, thread_num);
	...
	for (t = 0 ; t != thread_num ; ++t) 
	{
		...
		info[t].T5_bit_width      = T5_bit_width;
		...
		info[t].T5_selevitity     = T5_selevitity;
